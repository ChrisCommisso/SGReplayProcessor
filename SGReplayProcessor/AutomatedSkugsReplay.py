...
class MyServer(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        if len(unresolvedReplayData) > 0:
            z = open(r"C:\Users\sgrep\Documents\Skullgirls\Replays_SG2EPlus\76561199438243048\round_0001.rnd", 'wb')
            c = open(r"C:\Users\sgrep\Documents\Skullgirls\Replays_SG2EPlus\76561199438243048\round_0001.ini", 'w')
            z.write(unresolvedReplayData[currentIndex][3])
            c.write(unresolvedReplayData[currentIndex][4])
            z.close()
            c.close()
        ...
        if len(unresolvedReplayData) > 0:
            ...
            os.startfile(r"SuperCoolCode.lnk")
            time.sleep(5)
            SetForegroundWindow(find_window(title='Skullgirls Encore'))
            time.sleep(2)
            gamepad.press_button(button=vg.XUSB_BUTTON.XUSB_GAMEPAD_A)
...
if __name__ == "__main__":        
    ...
    connection = pyodbc.connect('driver=MySQL ODBC 8.0 Unicode Driver', host='localhost', database='replaydatabase', user='root', password='VictoryOverEvil8610')
    ...
    try:
        ...
        os.startfile(r"CoolAndProgramy.url")
        time.sleep(10)
        global unresolvedReplayData
